...
def get_config():
    ...
    arch.num_levels = 6
    arch.base_resolution = 4
    arch.feature_dim = 48
    arch.ndim = 1
    arch.interp_method = 'linear'
    arch.hidden_mult = 2
    arch.time_embed_dim = 64
    arch.max_period = 1.0
    arch.hidden_mult = 2
    arch.x_min = jnp.array([-1.0])
    arch.x_max = jnp.array([1.0])
    # arch.num_layers = 4
    # arch.hidden_dim = 256
    arch.out_dim = 1
    ...
    weighting.init_weights = ml_collections.ConfigDict({"ics": 1.0, "res": 1.0, 'bc': 1.0, 'bcx': 1.0})
