...
def get_config():
    ...
    wandb.name = "grid_1+1D"
    wandb.tag = None

    # Arch: Time-Dependent Multi-Res Grid
    config.arch = arch = ml_collections.ConfigDict()
    arch.arch_name = "TimeDependentPINN"
    arch.num_levels = 7             # 4-5 levels is usually sufficient for 1D AC; 7 might be overkill/slower
    ...
    optim.decay_rate = 0.95         # S slightly slower decay helps grid methods
    ...
    weighting.init_weights = ml_collections.ConfigDict({"ics": 0.1, "res": 1.0}) # Emphasize ICs slightly
