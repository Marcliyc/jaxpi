...
def get_config():
    ...
    arch.gaussian = ml_collections.ConfigDict()
    arch.gaussian.num_levels = 7             # 4-5 levels is usually sufficient for 1D AC; 7 might be overkill/slower
    arch.gaussian.base_resolution = 4        # Start slightly coarser
    arch.gaussian.feature_dim = 48           # 256 is very heavy for 1D; 64 or 128 is usually enough
    arch.gaussian.ndim = 1
    arch.gaussian.grid_range = 2
    arch.gaussian.grid_shift = 1
    arch.gaussian.shared = True
    ...
    training.max_steps = 200000     # Grid methods often converge faster than MLPs
    training.batch_size_per_device = 4096

    # Weighting
    config.weighting = weighting = ml_collections.ConfigDict()
    weighting.scheme = 'ntk'  # 'ntk' is heavy; grad_norm is a good balance
    weighting.init_weights = ml_collections.ConfigDict({"ics": 10.0, "res": 1.0}) # Emphasize ICs slightly
